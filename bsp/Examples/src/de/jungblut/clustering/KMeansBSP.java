...
public final class KMeansBSP
	...
	public final void setup(
		...
		String distanceClass = peer.getConfiguration().get(
				DISTANCE_MEASURE_CLASS);
	...
	private long updateCenters(
		...
		for (int i = 0; i < msgCenters.length; i++) {
			...
			if (msgCenters[i] != null) {
				double calculateError = oldCenter.subtract(msgCenters[i]).abs()
						.sum();
	...
	private void assignCenters(
		...
		if (cache == null) {
			...
			while (peer.readNext(key, value)) {
				assignCentersInternal(newCenterArray, summationCount, key
						.getVector().deepCopy());
			}
		} else {
			// if our cache is enabled but empty, we have to read it from disk
			// first
			if (cache.isEmpty()) {
				final NullWritable value = NullWritable.get();
				final VectorWritable key = new VectorWritable();
				while (peer.readNext(key, value)) {
					...
					assignCentersInternal(newCenterArray, summationCount,
							deepCopy);
				}
			} else {
		...
		if (peer.getPeerName().equals(peer.getPeerName(0))) {
			...
			if (pathString != null) {
				final SequenceFile.Writer dataWriter = SequenceFile
						.createWriter(FileSystem.get(conf), conf, new Path(
								pathString), VectorWritable.class,
	...
	public static void main(String[] args) throws IOException,
		...
		Path centerOut = new Path(
				"files/clustering/in/center/center_output.seq");
		...
		// if you're in local mode, you can increase this to match your core
		// sizes
		conf.set("bsp.local.tasks.maximum", "2");
		// deactivate (set to false) if you want to iterate over disk, else it
		// will
	...
	private static void prepareInput(int count, int k, int dimension,
		...
		for (int i = 0; i < count; i++) {
			...
			VectorWritable vector = new VectorWritable(new DenseDoubleVector(
					arr));
